import os
import json


def get_comments_by_page(page: int, places_per_page: int, address: str) -> list[str]:
    all_comments: dict[str, list[str]]
    with open("src/tg_bot/test_utils/comments/comments.json") as file:
        all_comments = json.load(file)
    return all_comments.get(address)[
        (page - 1) * places_per_page : page * places_per_page
    ]


def get_all_comments(address: str) -> list[str]:
    all_comments: dict[str, list[str]]
    with open("src/tg_bot/test_utils/comments/comments.json") as file:
        all_comments = json.load(file)
    return all_comments.get(address)


if __name__ == "__main__":
    print(
        get_comments_by_page(1, 4, "Салон красоты · Москва, Ленинградское шоссе, 8к3")
    )
