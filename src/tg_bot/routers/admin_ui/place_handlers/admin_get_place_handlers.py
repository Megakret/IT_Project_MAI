...
from tg_bot.routers.role_model_fsm.admin_fsm import AdminGetPlaceFSM
from tg_bot.keyboards import place_manager_kb

router = Router()
geosuggest_selector = GeosuggestSelector(AdminGetPlaceFSM.choose_place)


@router.message(
    or_f(Command("exit"), F.text == "Назад"),
    or_f(AdminGetPlaceFSM.enter_place, AdminGetPlaceFSM.choose_place),
)
async def exit_handler(message: Message, state: FSMContext):
    ...
    await state.set_state(AdminGetPlaceFSM.place_state)


@router.message(F.text == "Найти место", AdminGetPlaceFSM.place_state)
async def start_handler(message: Message, state: FSMContext):
    await get_place_funcs.start_get_place_function(message)
    await state.set_state(AdminGetPlaceFSM.enter_place)


@router.message(AdminGetPlaceFSM.enter_place, F.text)
...
@router.callback_query(AdminGetPlaceFSM.choose_place, F.data.contains(KEYBOARD_PREFIX))
